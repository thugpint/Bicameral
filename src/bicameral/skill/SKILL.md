...
1. Call `bicameral_execute(run_id, step_id)`.
   - **Delegated**: the Editor has already edited the files. You get the diff and the verification output.
   - **Routed to you**: make the edit yourself with your tools, minimal and exactly per the step, then call `bicameral_check(run_id, step_id)` to get the diff and verification output.
...
Report to the user: outcome, each step with who executed it and how many attempts, verification result, editor cost, and any lessons stored. Keep it short.
